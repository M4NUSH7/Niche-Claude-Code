...
**On a large/unfamiliar repo (optional, when graphify is present):** build a
code-graph once with `graphify update <repo>`, then answer "where is X used"
/ "how does X connect to Y" via `graphify query`/`path`/`explain` against
that graph instead of blind full-file reads or broad greps. Complements
grep-before-read above rather than replacing it - see the `graphify` skill.
...
## Content-type compression rules: what to compress, what to protect

RTK compresses command *output*; the rules below are guidance for judging *when compression of any kind (RTK or manual summarizing) is appropriate at all* versus when the content must stay raw. Apply by content type:

- **JSON/structured data**: preserve keys/structure; long string *values* are the compressible part, not the shape. Summarize repeated array elements rather than pasting all of them.
- **Code**: keep imports, function/class signatures, and type/interface defs intact; bodies and comments are what can be condensed when only the shape matters. If the task is a code *question* (debugging, "what does this do", review), keep the full surrounding thread uncompressed - signatures without bodies are not enough context to reason about a bug.
- **Logs**: keep timestamps, log levels, and every error/warning line; drop only repetitive non-error lines (e.g. identical heartbeat/poll lines). See "NEVER Read Entire Log Files" above for the filtering commands.
- **Search/grep results**: keep the `file:line:content` structure - it is already the compressed form; do not further summarize away the line numbers.

**Skip compression entirely below a size floor** - it costs more (in judgment/tool-call overhead) than it saves: payloads under ~200 tokens, or arrays/lists under ~5 items. Just show them as-is.

**Protect, never compress or drop:**
- Errors, stack traces, and exceptions - full text, always. This is the skill's existing "never compress error diagnoses" rule (see `references/output-style.md`); the content-type rules above reinforce it rather than override it for any content type.
- Function/method signatures and type definitions.
- The last few turns of an active conversation thread, and any thread that is actively debugging a specific piece of code - compressing recent context or an in-progress code discussion destroys exactly the material still being reasoned about.

**When to use `rtk proxy <cmd>` (raw, uncompressed) instead of the default compressing proxy**: any time the *next* step depends on exact byte-for-byte output - diffing raw output against a reference, feeding output into another exact-match tool, or debugging RTK's own filtering. Otherwise the default compressing path is correct; `rtk proxy` is the deliberate escape hatch, not the default.
