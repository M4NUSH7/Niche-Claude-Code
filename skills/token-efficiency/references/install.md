...
1. [OK] **Rust Token Killer** (this project) - LLM token optimizer
   - Repos: `rtk-ai/rtk`
   - Has `rtk gain` command for token savings stats

2. [FAIL] **Rust Type Kit** (reachingforthejack/rtk) - DIFFERENT PROJECT
...
WARNING: **WARNING**: `cargo install rtk` from crates.io might install the wrong package. Always verify with `rtk gain`.
...
#### From old hook with inline logic (pre-0.24) - WARNING: Breaking Change
...
rtk init --show
# Should show: [OK] Hook: ... (thin delegator, up to date)
```
...
rtk git add .         # -> "ok OK"
rtk git commit -m "msg"  # -> "ok OK abc1234"
rtk git push          # -> "ok OK main"
```
...
WARNING: **If you installed the wrong rtk (Type Kit)**, see [TROUBLESHOOTING.md](docs/TROUBLESHOOTING.md#problem-rtk-gain-command-not-found)
