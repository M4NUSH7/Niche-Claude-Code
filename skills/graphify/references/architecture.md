# Architecture (distilled from graphify's ARCHITECTURE.md, commit c83085c)

Read this when you need to know how the graph is built internally, what a
node/edge looks like on disk, or how to add a new language extractor. Not
needed for day-to-day query/path/explain navigation - see SKILL.md for that.
...
See the upstream `SECURITY.md` for the full threat model (not vendored here).

## Testing (upstream repo only - not applicable to using the installed CLI)

One test file per module under `tests/`. Run with `pytest tests/ -q`. All tests
are pure unit tests - no network calls, no file system side effects outside
`tmp_path`.
